<?php
/**
 * radixetcltd Plugin Install Class
 *
 * @package  radixetcltd
 * @since    2.2.0
 */
...
if ( ! class_exists( 'radixetcltd_Plugin_Install' ) ) :
	/**
	 * The radixetcltd plugin install class
	 */
	class radixetcltd_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			wp_enqueue_script( 'radixetcltd-plugin-install', get_template_directory_uri() . '/assets/js/admin/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $radixetcltd_version, 'all' );

			wp_enqueue_style( 'radixetcltd-plugin-install', get_template_directory_uri() . '/assets/css/admin/plugin-install.css', array(), $radixetcltd_version, 'all' );
		}
			...
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						...
						'url'     => '#',
						'classes' => array( 'radixetcltd-button', 'disabled' ),
					);
					...
					$button = array(
						...
						'url'     => $url,
						'classes' => array( 'radixetcltd-button', 'activate-now' ),
					);
					...
					$button = array(
						...
						'url'     => $url,
						'classes' => array( 'radixetcltd-button', 'sf-install-now', 'install-now', 'install-' . $plugin_slug ),
					);
...
return new radixetcltd_Plugin_Install();
