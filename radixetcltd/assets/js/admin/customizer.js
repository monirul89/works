...
( function( wp, $ ) {
	...
	api.SFGuidedTour = {
		...
		_hideTour: function( remove ) {
			...
			$( 'body' ).addClass( 'sf-exiting' ).on( 'animationend.radixetcltd webkitAnimationEnd.radixetcltd', function() {
				$( this ).removeClass( 'sf-exiting' ).off( 'animationend.radixetcltd webkitAnimationEnd.radixetcltd' ).addClass( 'sf-hidden' );
				self.$container.hide();
		...
		_revealTour: function() {
			...
			$( 'body' ).addClass( 'sf-entering' ).on( 'animationend.radixetcltd webkitAnimationEnd.radixetcltd', function() {
				$( this ).removeClass( 'sf-entering' ).off( 'animationend.radixetcltd webkitAnimationEnd.radixetcltd' );
		...
		_moveContainer: function( $selector ) {
			...
			this.$container.addClass( 'sf-moving' ).css({ 'transform': 'translateY(' + parseInt( position, 10 ) + 'px)' }).on( 'transitionend.radixetcltd', function() {
				self.$container.removeClass( 'sf-moving' );
				self.$container.off( 'transitionend.radixetcltd' );
			} );
		},
