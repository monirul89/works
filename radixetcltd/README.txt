...
radixetcltd is the perfect theme for your next WooCommerce project.

== Description ==

radixetcltd is the perfect theme for your next WooCommerce project. Designed and developed by WooCommerce Core developers, it features a bespoke integration with WooCommerce itself plus many of the most popular customer facing WooCommerce extensions. There are several layout & color options to personalize your shop, multiple widget regions, a responsive design and much more. Developers will love its lean and extensible codebase making it a joy to customize and extend. Looking for a WooCommerce theme? Look no further!

For more information about radixetcltd please go to https://woocommerce.com/radixetcltd/.

For even more customization, check out radixetcltd extensions https://woocommerce.com/product-category/radixetcltd-extensions/ and radixetcltd child themes https://woocommerce.com/product-category/themes/radixetcltd-child-theme-themes/.

== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in radixetcltd in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.
4. Go to https://docs.woocommerce.com/documentation/themes/radixetcltd/ guides on how to customize this theme.
...
radixetcltd is based on Underscores https://underscores.me/, (C) 2012-2017 Automattic, Inc.
...
radixetcltd serves fonts via the hosted Google Fonts service. radixetcltd does not share any data with Google directly.
...
= 2.4.3 - 2019-02-05 =
* Feature - Replicate radixetcltd's layout logic in the block editor. Wide and Full-width options are made available only if supported.
* Enhancement - Product pagination is now circular and skips hidden products.
* Tweak - Remove unused variables from `inc/customizer/class-radixetcltd-customizer.php`.
* Tweak - Refactored "Posted on" section in `radixetcltd_post_meta()`.
* Tweak - Standardize mixing of static/non static function calls in the `radixetcltd_Customizer` class.
...
* Fix - When outputting custom styling for extensions, check if the object `$radixetcltd` is in the expected format and has right properties.
* Dev - Babel added as a dependency.
* Dev - New `assets/js/src/editor.js` file.
* Dev - Revert filter name. `radixetcltd_woocommerce_customizer_css` reverted to `radixetcltd_customizer_woocommerce_css`.
* Dev - New hooks added to post header section: `radixetcltd_post_header_before`, `radixetcltd_post_header_after`.

= 2.4.2 - 2018-12-11 =
* Fix - Load in `functions.php` missing `radixetcltd-woocommerce-functions.php` file required for compatibility with Jetpack's Infinite Scroll feature.
...
* Fix - Moved margin from `.woocommerce-breadcrumb` to the new `.radixetcltd-breadcrumb` wrapper.
...
= 2.2.7 - 2018-01-23 =
* Fix - Reverted Chromium overflow fix introduced in 2.2.6 due to incompatibility with other radixetcltd products.
* Fix - Jetpack Google Translate widget styling.
* Tweak - Improved Grouped products table styling.
* Dev - Added filters; `radixetcltd_custom_logo_args`, `radixetcltd_register_nav_menus`, `radixetcltd_html5_args`, and `radixetcltd_site_logo_args`.
...
* Fix - Fixed typo in the `radixetcltd_WooCommerce` class, `thumbnail_columns` method description. Kudos [@cryptexvinci](https://github.com/cryptexvinci).
...
* Dev - Removed sticky cart. See [this issue](https://github.com/woocommerce/radixetcltd/issues/637) for more information.
...
* Dev - New hook to filter sidebar regions: `radixetcltd_sidebar_args`.
...
* Tweak - Removed Structured Data. See [this comment](https://github.com/woocommerce/radixetcltd/issues/620#issuecomment-313629032).
...
* Dev - New hook after body tag: `radixetcltd_before_site`.
...
* Dev - Added actions; `radixetcltd_jetpack_infinite_scroll_before`, `radixetcltd_jetpack_infinite_scroll_after`, `radixetcltd_jetpack_product_infinite_scroll_before` and `radixetcltd_jetpack_product_infinite_scroll_after`.
* Dev - Added filters; `radixetcltd_footer_widget_rows` and `radixetcltd_footer_widget_columns` to easily control the number of widget rows / columns in the footer.
...
* Dev - Upsells columns filterable via `radixetcltd_upsells_columns`.
* Dev - Added a column wrapper to product loops. To adjust the layout you now only need to filter `radixetcltd_loop_columns`.
...
* Dev - Refactored `radixetcltd_get_content_background_color()` to account for and give priority (over radixetcltd Designer) to radixetcltd Powerpack.
* Dev - Deprecated `is_woocommerce_activated()` and made it pluggable.
* Dev - Added `radixetcltd_is_woocommerce_activated()`.
...
* Dev - `radixetcltd_post_thumbnail()` is now hooked in to `radixetcltd_post_content_before`.
* Dev - Added `radixetcltd_post_content_before` and `radixetcltd_post_content_after` actions.
* Dev - `radixetcltd_post_thumbnail()` now provides a default size.
...
* Dev - Restructured the way objects are initiated. Objects are now initiated into an accessible global $radixetcltd for access to hooks. Kudos [@jtsternberg](https://github.com/jtsternberg).
...
* Dev - Made `radixetcltd_primary_navigation_wrapper()` and `radixetcltd_primary_navigation_wrapper_close()` pluggable.
...
* Dev - Deprecate `radixetcltd_html_tag_schema()`.

= 2.0.3 - 2016-06-02 =
* Fix - Fatal error when using radixetcltd without WooCommerce.
...
* Dev - Added `radixetcltd_sticky_order_review` filter.
* Dev - Added `radixetcltd_structured_data` filter for structured data customisation.
...
* Dev - Apply Customizer setting defaults on `customize_register` to account for child themes filtering `radixetcltd_setting_default_values`.
* Dev - Fix typo in the `radixetcltd_homepage_after_best_selling_products` action name.
...
* Feature - oEmbed styling to match radixetcltd aesthetic.
* Feature - The welcome screen now dynamically pulls in new products.
* Feature - Added `radixetcltd_header_styles()` for the header image/styles along with the `radixetcltd_header_styles` filter to customise the treatment of header styles.
...
* Dev - Deprecated `radixetcltd_categorized_blog()`.
* Dev - Deprecated `radixetcltd_sanitize_hex_color()`.
* Dev - Deprecated `radixetcltd_sanitize_layout()`.
...
= 1.6.0 - 2016-01-05 =
* Feature - [radixetcltd Mega Menus](https://woocommerce.com/products/radixetcltd-mega-menus/) integration.
...
* Tweak - Added new function `radixetcltd_star_rating_script`. Outputs JavaScript for the new star rating input while we wait for WooCommerce 2.5 to be released.
...
* Dev - Added some new hooks to the homepage template tags; `radixetcltd_homepage_after_product_categories_title`, `radixetcltd_homepage_after_recent_products_title`, `radixetcltd_homepage_after_featured_products_title`, `radixetcltd_homepage_after_popular_products_title`, `radixetcltd_homepage_after_on_sale_products_title`
* Dev - Styling for 6 column product layouts.
* Dev - Updates FontAwesome to version 4.5.0.

= 1.5.3 - 2015-11-13 =
* Fix - radixetcltd will now automatically load a child themes stylesheet.
...
* Dev - `radixetcltd_display_comments` is now hooked into `radixetcltd_single_post_after` with priority 20 (was 10).
* Localization - All translations are now managed on [WordPress.org](https://translate.wordpress.org/projects/wp-themes/radixetcltd)
...
* Dev - The default layout is now  filterable via `radixetcltd_default_layout`.
...
* Dev - Added `radixetcltd_sanitize_checkbox()` sanitization function.
* Dev - Added `radixetcltd_Custom_Radio_Image_Control` class for creating radio image controls in the Customizer.
* Dev - Added `radixetcltd_post_thumbnail()`.
* Dev - Renamed `do_shortcode_func()` to `radixetcltd_do_shortcode()`.
...
* Dev - `$radixetcltd_version` global when using a child theme.

= 1.4.5 - 2015-05-14 =
* Fix - Use the correct escaping function in `radixetcltd_product_categories()`.
...
* Dev - Made `Layout_Picker_radixetcltd_Control` class pluggable. Props @niravmehta
...
* Tweak - Skip links are now a function (`radixetcltd_skip_links()`) hooked into `radixetcltd_header`.
...
* Dev - Fixed typo in classname: `radixetcltd-feautred-products` is now `radixetcltd-featured-products`.
...
* Dev - Added `radixetcltd_related_products_args` filter.
...
* Fix - `radixetcltd_menu_toggle_text` filter. Props [jesinwp](https://github.com/jesinwp).
...
* Tweak - `mark` styling.
* Dev - Added `radixetcltd_sanitize_choices()`.
...
* Dev - `radixetcltd_header_cart()` is now pluggable.
...
* Dev - Added `radixetcltd_menu_toggle_text` filter.
* Dev - Updated how WooCommerce styles are dequeued.

= 1.2.3 - 2015-01-07 =
* Fix - Button border in WooCommerce messages when using the radixetcltd Designer extension.
* Tweak - Stock icon.
...
* Dev - Made `radixetcltd_product_search()` pluggable.
...
* Dev - Added `radixetcltd_single_post_posted_on_html` filter.
...
* Dev - Added `radixetcltd_copyright_text` filter.
* Dev - Tweaked how radixetcltd determines whether Customizer logic should be loaded.
* Dev - Added `radixetcltd_after_footer` action.
...
* Dev - Added `radixetcltd_product_thumbnail_columns` filter.
